          ...
          <BoxUi
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <Controller
                name="fullName"
                control={control}
                render={({field}) => <InputUi {...field} label={'Full Name'} className={`mt-5 ${classes.inputStyle}`}/>}
              />
              <Controller
                name="email"
                control={control}
              ...
              <Controller
                name="referralCode"
                control={control}
                render={({field}) => <InputUi {...field} label={'Referral Code'}
                                              className={`mt-5 ${classes.inputStyle}`}/>}
              />
              <div className='flex items-center mt-3 '>
                <FormGroup>
                  <FormControlLabel className={'text-[14px] font-[400] mr-1 ' + classes.textColor}
                                    control={<Checkbox onChange={() => setChecked(!checked)} size={'small'}/>} label="I agree to the"/>
                </FormGroup>
                <ButtonUi disableFocusRipple disableTouchRipple disableRipple className={'p-0'} sx={{minWidth: 'auto'}}
                          color={'primary'}>
                  Terms & Conditions
                </ButtonUi>
              </div>
              <ButtonUi type={checked ? 'submit' : 'button'} disabled={!checked} variant={'contained'} className={`mt-3 ${classes.button}`}>
                Create Account
              </ButtonUi>
            </form>
          </BoxUi>
